...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                // .cors(cors -> cors.corsConfigurationSource(corsConfigurationSource)) 메소드 이름을 바꾸고 싶은 경우
                .csrf(CsrfConfigurer::disable) // 외부 post, put 요청을 허용하기 위함
                .logout(auth -> auth // 따로 컨트롤러 만들지 않아도 로그아웃 로직 사용가능(get)
                    .logoutUrl("/auth/logout") // 로그아웃 주소 설정
            ...
            .anyRequest().authenticated() // 위에 걸러지지 않은 api 는 모두 인증을 필요로함
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        ...
        config.setAllowedOriginPatterns(Arrays.asList("*")); // 모든 원본에서의 요청을 허용하는 설정이다
        ...
        source.registerCorsConfiguration("/**", config); //모든 url에 이 config를 추가하겠다
